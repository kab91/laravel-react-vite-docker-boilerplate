...
RUN apk add tidyhtml-dev libzip-dev zip
RUN docker-php-ext-install pdo pdo_mysql bcmath tidy zip

# Installing redis extension
RUN mkdir -p /usr/src/php/ext/redis \
    && curl -fsSL https://github.com/phpredis/phpredis/archive/6.0.2.tar.gz | tar xvz -C /usr/src/php/ext/redis --strip 1 \
